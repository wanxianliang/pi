	...
	agent = new Agent({
		...
		streamFn: async (model, context, options) => {
			...
			if (options?.sessionId === sessionManager.getSessionId()) {
				cacheWarmer.start({ model, context: llmContext, options: requestOptions }, cacheContextIsCurrent(model));
			}
			return modelRuntime.streamSimple(model, llmContext, requestOptions);
		},
