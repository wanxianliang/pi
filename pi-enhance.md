...
> **当前基准版本**：`upstream/main @ d1230ea20` (v0.86.0)  
...
| `packages/coding-agent/package.json` | 添加 `@earendil-works/pi-enhance-tui: ^0.86.0` 依赖（内部包版本对齐上游 0.86.0）及 `build:binary` 构建前置步骤 |
| `packages/coding-agent/install-lock/package-lock.json` | 记录 `@earendil-works/pi-enhance-tui` (0.86.0) 依赖锁信息 |
...
| `package-lock.json` | 注册 `@earendil-works/pi-enhance-tui` (0.86.0) workspace 软链接及依赖项 |
...
- `package-lock.json`、`install-lock/package-lock.json`、`npm-shrinkwrap.json`：注册并锁定本地 workspace 包 (0.86.0)。
